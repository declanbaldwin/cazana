(function() {
  ...
  calculateAverageNumberOfMilesAddedPerDay();

  let averageAnnualMilage = calculateAverageAnnualMilage();
  console.log(`Average Annual Mileage: ${averageAnnualMilage}`);

  let projectedMilage = calculateProjectedMileage();
  console.log(`Projected Milage: ${projectedMilage}`);

  function calculateProjectedMileage() {
    let averageMilagePerDay =
      arrayOfAverageNumberOfMilesAddedPerDay.reduce(add, 0) /
      arrayOfAverageNumberOfMilesAddedPerDay.length;

    let today = new Date();
    let dateOfLastMOT = new Date(history[0].date);

    let numberOfDaysSinceLastMOT = calculateDaysBetweenTwoDates(
      dateOfLastMOT,
      today
    );
    return (
      history[0].data.mileage + averageMilagePerDay * numberOfDaysSinceLastMOT
    );
  }

  function calculateAverageAnnualMilage() {
    let arrayOfAverageAnnualMileage = [];
    for (let i = 0; i < arrayOfAverageNumberOfMilesAddedPerDay.length; i++) {
      arrayOfAverageAnnualMileage.push(
        arrayOfAverageNumberOfMilesAddedPerDay[i] * 365
      );
    }

    return (
      arrayOfAverageAnnualMileage.reduce(add, 0) /
      arrayOfAverageAnnualMileage.length
    );
  }
  ...
  function calculateAverageNumberOfMilesAddedPerDay() {
    for (let i = 0; i < 4; i++) {
      ...
      if (numberOfMilesDrivenSinceLastMOT == 0) {
        ...
        averageNumberOfMilesAddedPerDay =
          (mileage2 - mileage1) / numberOfDaysBetweenTwoDates;
      }
      arrayOfAverageNumberOfMilesAddedPerDay.push(
        averageNumberOfMilesAddedPerDay
      );
    }
  }
